...
export default function CodeJar(editor, highlight, opt = {}) {
    ...
    function handleCommentCommand(event) {
        ...
        const binding = game.keybindings.get(CONSTANTS.MODULE_ID, "commentCommandMacroEditor")[0];
        if (event.code === binding?.key && binding?.modifiers.every((mod) => context.modifiers.includes(mod))) {
            ...
            const editPoints = [
                min,
                ...getAllIndexes(between, "\n")
                    .filter((i) => i !== 0 && i !== between.length - 1)
                    .map((i) => i + min + 1),
            ]
                .sort((a, b) => a - b)
                .reverse();
    ...
    function getSelection() {
        var _a;
        if (
            ((_a = editor.parentNode) === null || _a === void 0 ? void 0 : _a.nodeType) == Node.DOCUMENT_FRAGMENT_NODE
        ) {
